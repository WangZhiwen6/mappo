...
import mygym
from mygym.utils.constants import YEAR
...
#     """Convert a conf from json format in a dictionary of all possible environments.
    
#     Added Debug prints to help identify configuration mismatches.
#     """

#     configurations = {}

#     # --- DEBUG START: 打印当前正在处理的配置 ID ---
#     print("\n" + "="*60)
#     print(f"[DEBUG] 正在处理配置文件 ID Base: {conf.get('id_base', 'Unknown')}")
#     print("="*60)
...
#         print(f"[DEBUG ERROR] 解析 variables/meters/actuators 时出错: {e}")
#         raise e

#     # --- DEBUG START: 检查天气规格 ---
#     weather_spec = conf.get('weather_specification', {})
#     weather_files = weather_spec.get('weather_files', [])
#     weather_keys = weather_spec.get('keys', [])

#     len_files = len(weather_files)
#     len_keys = len(weather_keys)

#     print(f"[DEBUG] 天气文件列表长度 (weather_files): {len_files}")
#     print(f"[DEBUG] 天气ID键列表长度 (keys):          {len_keys}")

#     if len_files != len_keys:
#         print("\n[DEBUG ERROR] 检测到长度不匹配！详细数据如下:")
#         print("-" * 40)
#         print(f"weather_files ({len_files}):")
...
#         # 这里保留 assert 以阻止程序在错误配置下继续运行，
#         # 如果你希望程序即使出错也继续尝试（可能会报其他错），可以注释掉下面这行
#         assert len_files == len_keys, f'Weather files ({len_files}) and id keys ({len_keys}) must have the same len. Check debug output above.'
    
#     print("[DEBUG] 天气配置检查通过。")
#     # --- DEBUG END ---

#     weather_info = list(zip(weather_keys, weather_files))

#     variation = conf.get('variation')

#     for idx, (weather_id, weather_file) in enumerate(weather_info):
#         print(f"[DEBUG] 正在生成环境 #{idx+1}/{len(weather_info)}: ID={weather_id}, File={weather_file}")

#         # 1. 生成标准环境 (Continuous)
#         id_standard = 'Eplus-' + conf['id_base'] + '-' + weather_id + '-continuous-v1'

#         try:
#             action_space_obj = eval(conf['action_space'])
#             reward_obj = eval(conf['reward'])
#         except Exception as e:
#             print(f"[DEBUG ERROR] 解析 action_space 或 reward 字符串时出错: {e}")
#             print(f"  出错的值: action_space='{conf['action_space']}', reward='{conf['reward']}'")
#             raise e
...
#             'env_name': id_standard.replace('Eplus-', ''),
#             'config_params': conf.get('config_params')
#         }
#         configurations[id_standard] = env_kwargs

#         # 2. 生成随机变异环境 (Stochastic) - 如果存在 variation
#         if variation:
#             id_stochastic = 'Eplus-' + conf['id_base'] + '-' + \
...
#                 'reward_kwargs': conf['reward_kwargs'],
#                 'max_ep_data_store_num': conf['max_ep_data_store_num'],
#                 'env_name': id_stochastic.replace('Eplus-', ''),
#                 'config_params': conf.get('config_params')
#             }
#             configurations[id_stochastic] = env_kwargs_stoch
#             print(f"[DEBUG] 已生成随机变异环境: {id_stochastic}")

#     print(f"[DEBUG] 成功生成 {len(configurations)} 个环境配置。")
#     print("="*60 + "\n")
